...
public class EnemyDamageFlying : MonoBehaviour
{
    ...
    ControllerFlying enemyController;
    ...
    void Start()
    {
        ...
        enemyController = transform.parent.gameObject.GetComponent<ControllerFlying>();
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if ((sword = collision.gameObject.GetComponent<IDamageSword>()) != null)
        {
            ...
            // only recieve knockback when dead
            // 死んだ時だけノックバック受ける
            if(enemyController.EnemyHP <= 0)
			{
                enemyController.stopCoroutineFunc();
                rbody2d.gravityScale = 0.3f;
                KnockbackBig(knockback.x + 0.5f, 0, scaleX);
			}
