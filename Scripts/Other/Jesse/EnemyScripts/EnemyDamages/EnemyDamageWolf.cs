...
public class EnemyDamageWolf : MonoBehaviour
{
    ...
    ControllerWolf enemyController;
    ...
    void Start()
    {
        ...
        enemyController = transform.parent.gameObject.GetComponent<ControllerWolf>();
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if((sword = collision.gameObject.GetComponent<IDamageSword>()) != null)
        {
            ...
            magicController.SetManaSteal(mpSteal); // TP‰ñ•œ
            enemyController.EnemyHP -= Mathf.CeilToInt(damage * buff);
            coroutine = null;

            if(critFlag)
                coroutine = StartCoroutine(CritFlash()); // Ô‚­Œõ‚é            
            else
